...
class _PinCodeTextFieldState extends State<PinCodeTextField>
  ...
  List<BoxShadow>? _getBoxShadowFromIndex(int index) {
    if (((_selectedIndex == index) ||
            (_selectedIndex == index + 1 && index + 1 == widget.length)) &&
        _focusNode!.hasFocus) {
      return _pinTheme.selectedBoxShadow;
    }
  }

  TextStyle? _getTextStyleFromIndex(int index) {
    if (((_selectedIndex == index) ||
            (_selectedIndex == index + 1 && index + 1 == widget.length)) &&
        _focusNode!.hasFocus) {
      return _pinTheme.selectedTextStyle;
    } else if (_selectedIndex > index) {
      TextStyle? relevantActiveTextStyle;
      if (isInErrorMode) relevantActiveTextStyle = _pinTheme.errorTextStyle;
      return relevantActiveTextStyle;
    }
  }
  ...
  Widget _renderPinField({
    ...
    return widget.textGradient != null
        ...
        : Text(text,
            key: ValueKey(_inputList[index]),
            style: _getTextStyleFromIndex(index) ?? _textStyle);
  }

...
class SPNestedScrollView extends StatefulWidget {
  ...
  static SliverOverlapAbsorberHandle sliverOverlapAbsorberHandleFor(
    ...
    assert(
      target != null,
      'SPNestedScrollView.sliverOverlapAbsorberHandleFor must be called with a context that contains a SPNestedScrollView.',
    );
...
/// [NestedScrollView], you can get its [SPNestedScrollViewState] by supplying a
/// `GlobalKey<SPNestedScrollViewState>` to the [SPNestedScrollView.key] parameter).
///
/// {@tool dartpad}
...
class SPNestedScrollViewState extends State<SPNestedScrollView> {
  ...
  ///    sliver(s) contained in [SPNestedScrollView.headerSliverBuilder].
  ...
  ///    [ScrollView] contained in [SPNestedScrollView.body].
    ...
    return _InheritedNestedScrollView(
      ...
      child: Builder(
        builder: (BuildContext context) {
          ...
          var content = _SPNestedScrollViewCustomScrollView(
...
class _SPNestedScrollViewCustomScrollView extends CustomScrollView {
  const _SPNestedScrollViewCustomScrollView({
  ...
  void applyUserOffset(double delta) {
    ...
    if (_innerPositions.isEmpty) {
      ...
      if (innerDelta != 0.0) {
        ...
        // debugPrint("------------------------12>$outerDelta");

        var temDelta = outerDelta;
        if (outerDelta != 0.0)
          outerDelta -= _outerPosition!.applyClampedDragUpdate(outerDelta);
        // debugPrint("------------------------13>$outerDelta");
...
// SPNestedScrollView. It tracks the offset to use for those viewports, and knows
